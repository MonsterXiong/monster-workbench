...
- [ ] 是否遵守了项目现有目录结构和命名规范？
- [ ] 代码风格是否与周围代码一致？
- [ ] Page / 组件 / Store / Service / Rust Service / Repo 是否已按职责边界、复杂度和可拆分点做体量审查，而不是按行数机械拆分？
...
- [ ] release notes 写入 GitHub Actions `$GITHUB_OUTPUT` 是否继续走 `scripts/write-release-notes-output.js`，并能兼容 release notes 文件没有结尾换行的情况？
...
- [ ] Python sidecar 或 Rust AI 服务变更后，是否运行 `npm run check:architecture`、`npm run test:ai-sidecar`、`npm run test:ai-sidecar:stress`，并按需运行 Rust 检查？
- [ ] 新增长驻 worker、worker 池或二进制 sidecar 时，是否仍由 Rust Service 独占调度，并覆盖 requestId、capability、heartbeat/lease、取消确认、artifact 输出目录、日志脱敏、异常终态回写和 bundle resources / externalBin 配置？
