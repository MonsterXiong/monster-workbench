...
- [ ] 2026-06-13 已继续收口 `BasePagination`：保留项目汇总、边缘按钮、BaseSelect 页容量默认模式、空态和窄容器语义，同时补齐 Element Plus 原生 sizes 布局、`pageSizeControl="native"`、page-size 下拉 teleported / appendSizeTo / popperClass / popperStyle 透传和 `update:page-size` 事件；Playground 已覆盖原生页容量分页示例。
