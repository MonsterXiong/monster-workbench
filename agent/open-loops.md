...
- [ ] 2026-06-13 已继续收口 `BaseUpload` / `BaseAvatar` / `BaseEmpty`：`BaseUpload` 保留 FileList 校验、select/reject 和默认轻量模式，同时补齐 Element Plus 文件列表、列表类型、limit/exceed、preview/remove、drag 开关和实例方法；`BaseAvatar` 补齐 `fit`、`srcSet` 和 error 事件；`BaseEmpty` 补齐 `image` / `imageSize` 图片空态。Playground 已覆盖文件列表与限制、头像 fit 和图片空态示例。后续继续审计上传实际业务链路时，仍不得让页面绕过 store/service/Rust 边界直接做网络或文件系统能力。
