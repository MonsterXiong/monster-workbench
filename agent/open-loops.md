...
- [ ] 继续推进 Python workflow runtime：`generate_image_prompt` 已落地 task request/result、model_runs 审计、失败/取消/重试映射和 cancel checkpoint，下一步迁移 batch prompt worker 的 provider 调用。
...
- [ ] 后续继续同步 `docs/architecture-current-state.md`：当 batch worker 迁移落地后，更新 Rust / Python 执行面职责和 batch worker 迁移状态。
