...
- [ ] 基于 Rust TaskService / BatchJobService 边界评估，优先设计“Rust 控制面 claim/状态/事件 + Python 执行业务”的 batch worker 迁移协议。
...
- [ ] 基于本轮 TaskService / BatchJobService 边界评估，继续细化 Python workflow runtime 的任务输入、执行结果、model_runs 审计、取消 checkpoint 与失败恢复协议。
