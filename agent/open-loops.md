...
- [ ] 2026-06-12 已删除未被引用的历史 `docs/agents/*` 与 `docs/templates/*`；如后续继续扩写架构材料，优先更新 `docs/architecture-current-state.md` 和对应专题文档，不再新增平行路线图、阶段提示词、角色提示词目录或一次性执行包。
...
- [ ] 2026-06-12 本轮继续收口 `BaseButton`（共享 `toElementPlusSize()` 并校正图标按钮居中）、`BaseCheckbox` / `BaseRadioGroup` / `BaseSwitch`（共享尺寸映射并补齐 `xs` 表单密度示例）与 `BaseSlider`（Element Plus 手柄与轨道中心对齐）；后续继续审计表单、选择和表格体系的高阶能力，仍避免触碰 AI / Creative / `aiPage*` 页面。
- [ ] 2026-06-12 继续收口 `BaseNumberInput` / `BaseTextarea` / `BaseTagInput`：三者共享 `toElementPlusSize()` / `ProjectControlSize`，补齐 `xs` 密度和 Playground 真实示例；后续继续评估更高阶表单能力，不为换底座压缩现有 API。
- [ ] 2026-06-12 继续收口 `BaseProgress`：在保留项目语义色、区间、缓冲条、loading / indeterminate 和 ARIA 文本的基础上，补齐 Element Plus `line` / `circle` / `dashboard` 形态，并在 Playground 增加环形、仪表盘、自定义直径和端点样式示例；后续继续审计剩余高频展示组件。
- [ ] 2026-06-12 继续收口 `BaseTooltip`：在保留项目内容 slot、受控 `open`、视口避让和可访问描述的基础上，补齐 Element Plus / Popper 的 `*-start/end` placement、`hover/focus/click/contextmenu` trigger、`effect="light"`、`showArrow`、`enterable`、`autoClose`、`fallbackPlacements` 和 `strategy`；Playground 已增加触发方式、浅色提示、无箭头和自动关闭示例。
- [ ] 2026-06-12 已继续收口 `AppImageUploader`（`ElImage` / `ElButton` 外观，上传仍走 `fileManagerStore.uploadSelectedImage()`）、`AppPathSelector`（`ElInput` append 按钮）、`BaseCopyButton`、`BaseCommandPalette`、`BaseToast`、`ConfirmDialog`、`BaseActionMenu` / `BaseAlert` / `BaseDialog` / `BaseDrawer` / `BaseMessage` / `BasePageHeader` / `BaseFilterBar` / `BaseSelectionBar` / `BaseDateRange`（关闭、返回、筛选清空、选择清空、菜单触发等小动作统一经 `BaseButton`，日期面板补齐 shortcuts / unlink panels / single panel 透传）、`BaseBreadcrumb`（折叠路径可选 `ElDropdown` 下拉菜单，默认展开模式保持兼容）、`BaseBadge`（closable 改用 `ElTag` close）、`BaseDetailCard` / `BaseInfoCard` / `BaseStatCard` / `BaseStatusDot`（`ElCard` / `ElTag` / `ElBadge` 等展示底座）、`BaseFieldGroup` / `BasePanel` / `BaseDataState`（`ElCard` 外壳）、`BaseKeyValueList`（loading/empty 切到 `ElSkeleton` / `ElEmpty`）、`BaseList`（默认 loading/empty 复用 `BaseLoading` / `BaseEmpty`）、`BaseSearchInput`（共享 `BaseInput` 的 Element Plus 尺寸映射，补齐 `xs` 搜索尺寸）、`BaseSegmented`（共享尺寸映射，补齐 `xs` 迷你分段，默认继续走 `ElSegmented`）、`BaseSlider`（共享尺寸映射，补齐 `xs` / `lg` 滑块尺寸）、`BasePagination`（共享尺寸映射，透传跳页、背景、单页隐藏、自定义 layout / pagerCount）、`BaseTable` / `BaseDataTable`（透传排序、固定列和 `sort-change`）与 `BaseForm` / `BaseFormItem`（透传 `prop` / `rules`，暴露 `validate` / `resetFields` / `clearValidate` 等实例方法）；后续继续审计表格体系和表单体系更高阶能力，仍避免盲目触碰 AI / Creative / `aiPage*` 页面。
