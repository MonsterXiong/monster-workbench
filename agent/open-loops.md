...
- [ ] 继续推进 Python workflow runtime：`generate_image_prompt` 已落地 task request/result、model_runs 审计、失败/取消/重试映射和 cancel checkpoint；`demo.image.prompt` 与 `demo.image.generate` provider 执行已迁到 sidecar workflow，下一步重点是 batch sidecar cancel checkpoint、sidecar lifecycle 复用、预算/超时协议和正式 workflow 类型命名。
