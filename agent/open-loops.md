...
- [ ] 继续收口 Creative service / backend：2026-06-12 复核确认 Rust `TaskService` 短期可保留 task/asset/event 可信入口，先冻结 `run_review_asset_quality_stub` 的业务扩展；`BatchJobService` 的 supervisor / prompt worker shell / image worker shell 也短期保留在 Rust，不再新增正式生产 worker 分支。2026-06-12 继续复核后已把执行口径、worker ownership / lease migration 草案和 Rust-owned localhost sidecar control API contract 补入 `docs/ai/workflow-runtime-boundary.md` 12.8-12.12；下一步若进入实现，必须先做 migration 旧库兼容回归、lease-aware claim/heartbeat/complete repo 测试和 result settle contract 验收，正式 review/revision 与复杂业务策略迁入 Python workflow runtime。
