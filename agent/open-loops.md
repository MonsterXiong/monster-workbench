...
- [ ] 图片工作台后端后续扩展前继续拆：repo 已拆出 `image_workbench_types.rs` 与 `image_workbench_repo_tests.rs`，service 已拆出 `image_workbench_asset_policy.rs`；2026-06-19 增 `image_workbench_row_mapper.rs` / `image_workbench_task_transition.rs` / `image_workbench_runner.rs`；2026-06-20 把 repo 私有只读查询 helper 抽到 `image_workbench_query.rs`，repo 降到 856 行恢复体量预算。后续增加受控导入参考图/蒙版、资产删除文件清理、ZIP 打包策略前，再拆 mutations、asset recording builder。
...
- [x] 2026-06-20 图片工作台 grouped job + 版本链已真正写入（契约零变更）：`repo.create_job` 在同事务内建一个资产组（`source_asset_id` 有值→`type=rerun`，否则 `fresh`；`base_prompt=prompt`、`count=quantity`），该 job 的 N 个 task 全挂这个组并写 `variant_index=0..N`；`repo.record_asset` 经 `image_workbench_query::resolve_lineage` 从 `job.source_asset_id` 派生版本链——`parent=源asset.id`、`root=源asset.root??源asset.id`（链根向上收敛，对齐前端 `resolveRootAssetId`）、`version=源asset.version+1`，`group_id` 跟随 task；显式传入值（测试/导入）优先于派生值。`CreateImageWorkbenchJobRequest`/`NewImageWorkbenchJob`/runner/前端/Python 均未改。repo + service 双层单测覆盖：create_job 建组+变体序号、fresh/rerun type、全新 job 无版本链、三级复跑链 α→β→γ root 收敛+version 递增、端到端 worker 生成后资产带 group/版本链。下一轮：rating/分组 command + 前端接入（把 `imageWorkbenchReview.ts` 的前端推断换成读后端 `groupId`/`parentAssetId`/`rootAssetId`/`versionIndex`/`rating`）、`archived_at_ms`/`deleted_at_ms` 与损坏状态、failure_type 业务写入。
