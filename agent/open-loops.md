...
- [ ] 2026-06-12 已继续收口 `AppImageUploader`（`ElImage` / `ElButton` 外观，上传仍走 `fileManagerStore.uploadSelectedImage()`）、`AppPathSelector`（`ElInput` append 按钮）、`BaseCopyButton`、`BaseCommandPalette`、`BaseToast`、`ConfirmDialog`、`BaseActionMenu` / `BaseAlert` / `BaseDialog` / `BaseDrawer` / `BaseMessage` / `BasePageHeader` / `BaseFilterBar` / `BaseSelectionBar` / `BaseDateRange`（关闭、返回、筛选清空、选择清空、菜单触发等小动作统一经 `BaseButton`，日期面板补齐 shortcuts / unlink panels / single panel 透传）、`BaseBadge`（closable 改用 `ElTag` close）、`BaseDetailCard` / `BaseInfoCard` / `BaseStatCard` / `BaseStatusDot`（`ElCard` / `ElTag` / `ElBadge` 等展示底座）、`BaseFieldGroup` / `BasePanel` / `BaseDataState`（`ElCard` 外壳）、`BaseKeyValueList`（loading/empty 切到 `ElSkeleton` / `ElEmpty`）、`BaseList`（默认 loading/empty 复用 `BaseLoading` / `BaseEmpty`）、`BaseSearchInput`（共享 `BaseInput` 的 Element Plus 尺寸映射，补齐 `xs` 搜索尺寸）、`BaseSlider`（共享尺寸映射，补齐 `xs` / `lg` 滑块尺寸）、`BasePagination`（共享尺寸映射，透传跳页、背景、单页隐藏、自定义 layout / pagerCount）、`BaseTable` / `BaseDataTable`（透传排序、固定列和 `sort-change`）与 `BaseForm` / `BaseFormItem`（透传 `prop` / `rules`，暴露 `validate` / `resetFields` / `clearValidate` 等实例方法）；后续继续审计表格体系和表单体系更高阶能力，仍避免盲目触碰 AI / Creative / `aiPage*` 页面。
