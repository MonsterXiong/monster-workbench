...
- [ ] 图片工作台后端后续扩展前继续拆：repo 已拆出 `image_workbench_types.rs` 与 `image_workbench_repo_tests.rs`，service 已拆出 `image_workbench_asset_policy.rs`；2026-06-19 增 `image_workbench_row_mapper.rs` / `image_workbench_task_transition.rs` / `image_workbench_runner.rs`，repo 980 / service 1417 行均位于体量基线内。后续增加搜索分页、受控导入参考图/蒙版、资产删除文件清理、ZIP 打包策略前，再拆 query / mutations、asset recording builder。
