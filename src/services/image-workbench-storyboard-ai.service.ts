...
const STORYBOARD_AI_DEFAULT_PREFIX =
  "参考图人物特征保持一致，保持同一张脸、同一五官比例、同一气质神韵，古风美女，电影级画面，细腻皮肤质感，东方审美，高级古装摄影，真实光影，精致服饰纹理。";
const STORYBOARD_AI_DEFAULT_NEGATIVE_PROMPT =
  "不要改变参考图人物五官，不要换脸，不要现代妆容，不要欧美五官，不要夸张网红脸，不要塑料皮肤，不要过度磨皮，不要畸形手指，不要多手多脚，不要低清晰度，不要廉价影楼风，不要现代建筑，不要现代服饰，不要文字水印，不要模糊面部。";
...
2. 分镜主体应围绕同一个参考图人物；变化集中在服装、情绪、动作、场景、镜头语言和故事氛围。
...
5. 如果原文有统一正向提示词，放到 prefix；如果没有，保持空字符串。
6. 如果原文有通用负面提示词，放到 negativePrompt；如果没有，保持空字符串。
7. 如果原文有编号、标题或明显段落，请按原顺序逐条保留，不要合并、不要遗漏；标题尽量使用原文标题，例如“01｜初见：柳岸回眸”。
8. 每个 scene 必须有 title、picturePrompt、cameraPrompt、emotionKeywords、referencePrompt。缺失字段可根据上下文补短句，但不要扩写成另一篇文章。
9. scenes 必须是非空数组；至少输出 1 个分镜。
...
2. 分镜围绕同一个参考图人物展开，默认主角是参考图人物；保持同一张脸、同一五官比例、同一气质神韵。
3. 故事要有短剧节奏：开场钩子、身份/关系冲突、误会或危机、反击、反转、情绪高潮、封面感收束。
4. 每条分镜都要能直接用于图片生成，变化集中在服装、情绪、动作、场景、镜头语言和故事氛围。
...
8. prefix 统一放保持参考人物一致性的正向前缀；negativePrompt 统一放避免换脸、现代元素、低质、畸形等负面词。
