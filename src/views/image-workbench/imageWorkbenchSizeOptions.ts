...
export function getImageSizeKindLabel(size: string, t: ImageTranslate) {
  const dimensions = parseDimensionsText(size);
  if (!dimensions) {
    return t("imageWorkbench.input.sizeKindCustom");
  }
  const ratio = getDimensionsRatio(dimensions);
  if (ratio === 1) {
    return t("imageWorkbench.input.sizeKindSquare");
  }
  if (ratio >= 3) {
    return t("imageWorkbench.input.sizeKindPanorama");
  }
  if (ratio <= 1 / 3) {
    return t("imageWorkbench.input.sizeKindVerticalPanorama");
  }
  return ratio > 1 ? t("imageWorkbench.input.sizeKindLandscape") : t("imageWorkbench.input.sizeKindPortrait");
}

export function buildImageSizeOption(size: string, t: ImageTranslate) {
  if (size === IMAGE_GENERATION_AUTO_SIZE) {
    const label = t("imageWorkbench.input.sizeAuto");
    return {
      label,
      selectedLabel: label,
      description: label,
