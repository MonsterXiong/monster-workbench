...
export const IMAGE_WORKBENCH_STORYBOARD_DEFAULT_PREFIX =
  "参考图人物特征保持一致，保持同一张脸、同一五官比例、同一气质神韵，古风美女，电影级画面，细腻皮肤质感，东方审美，高级古装摄影，真实光影，精致服饰纹理，服装材质清晰，环境层次完整，叙事感强，画面完成度高。";

export const IMAGE_WORKBENCH_STORYBOARD_DEFAULT_NEGATIVE_PROMPT =
  "不要改变参考图人物五官，不要换脸，不要现代妆容，不要欧美五官，不要夸张网红脸，不要塑料皮肤，不要过度磨皮，不要畸形手指，不要多手多脚，不要低清晰度，不要廉价影楼风，不要现代建筑，不要现代服饰，不要文字水印，不要模糊面部，不要错误服装时代，不要道具穿帮，不要背景混乱，不要多人身份混淆。";

const IMAGE_WORKBENCH_STORYBOARD_IDENTITY_RULE =
  "人物脸部五官、气质、发型识别点保持参考图一致，变化集中在服装、情绪、动作、场景、镜头语言和故事氛围。";

const IMAGE_WORKBENCH_STORYBOARD_REFERENCE_RULE =
  "参考图使用原则：人物参考用于锁定主角身份；场景参考用于空间、建筑、天气和光线；道具参考用于关键物件形态与材质；服装/风格参考用于时代感、衣料、纹样和配色；其他人物只作为配角关系参考，不得覆盖主角五官。";

const IMAGE_WORKBENCH_STORYBOARD_DIRECTOR_RULE =
  "以影视分镜标准组织画面，明确主体调度、动作瞬间、空间层次、前中后景关系、主光方向、氛围色彩、镜头焦段和景别，让每张图像像同一部古风电影中的连续剧照。";
...
export function buildImageWorkbenchStoryboardBatch(options: {
  ...
  scenes.forEach((scene) => {
    ...
    Array.from({ length: variants }, (_, index) => {
      ...
      taskPrompts.push(
        `${scene.prompt} 候选图 ${index + 1}/${variants}：保持同一分镜设定和参考图一致性，允许光影、表情细节、发丝、衣袂动态、景深和构图微差，输出可用于筛选的高质量候选。`
      );
      taskMetas.push({
        taskIndex,
