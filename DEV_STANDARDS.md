...
## 11. 自动更新、二级弹窗与递归防挂起保护规范 (最新增补)

为了在长久维护中保障桌面客户端的高性能、健壮性与精致感，必须遵守以下核心架构约束：
...
### B. 功能性二级配置弹窗 (Modal) 收纳原则
- **拒绝主视窗堆砌**：对于不影响核心操作流的辅助参数配置（如忽略目录列表、过滤阈值、递归层数上限等表单输入），禁止直接平铺摆放在主窗口区域。必须将其收纳在二级配置弹窗中，主界面仅保留一个精美的齿轮配置按钮，最大程度保障主窗口的专注度与极简美学。
- **遮罩与过渡动画**：所有弹窗遮罩层必须统一使用半透明且带高斯模糊的样式（如 `bg-slate-900/60 backdrop-blur-sm`），且内部 Card 的入场与离场必须配合 Vue `<transition>` 的缩放和不透明度缓动动效（如 `scale-95 opacity-0` ➔ `scale-100 opacity-100`），从而带给用户极致丝滑的视觉体验。
...
## 12. 文件与数据库关联一致性与自愈设计原则 (最新增补)

为了避免物理文件删除导致数据库中残留无效相对路径，进而在前端页面上渲染出破图、死链等极其低端的视觉与交互漏洞，必须落实以下规范：
...
- **关联路径自愈与数据库同步**：一旦用户确认强制删除，系统在执行底座物理删除文件之前，**必须首先通过 SQLite UPDATE 命令将数据库中所有引用的路径字段置空**（设置为 `NULL` 或空字串），彻底消除数据死链，保证系统层数据高度自愈。
