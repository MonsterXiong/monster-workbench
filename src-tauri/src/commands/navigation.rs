use crate::services::navigation_service::{
    NavigationItem, NavigationService, PagedResult, SortOrderItem,
};
use std::sync::Mutex;
use tauri::State;
...
#[tauri::command]
pub fn batch_delete_navigation(
    db_path: String,
    ids: Vec<i32>,
    state: NavState<'_>,
) -> Result<(), String> {
...
#[tauri::command]
pub fn increment_navigation_clicks(
    db_path: String,
    id: i32,
    state: NavState<'_>,
) -> Result<(), String> {
...
#[tauri::command]
pub fn get_navigation_categories(
    db_path: String,
    state: NavState<'_>,
) -> Result<Vec<String>, String> {
    let service = state.lock().unwrap_or_else(|e| e.into_inner());
    service
        .get_categories(&db_path)
        .map_err(|e| e.to_json_string())
}
...
#[tauri::command]
pub fn get_all_navigation_list(
    db_path: String,
    state: NavState<'_>,
) -> Result<Vec<NavigationItem>, String> {
    let service = state.lock().unwrap_or_else(|e| e.into_inner());
    service
